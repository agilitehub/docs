...
* Let's assume that I own a transportation company, at said company I do predictive modelling. I could use the **Data Mapping Module** to setup an `Excel to JSON` configuration and use that to translate the modelling data that I have in Excel into JSON so that I can send that along for someone else to use.
  ...
  This would make my life a lot easier, because instead of having to pass along an Excel via the cloud (or a hard drive), then have the other person convert it. I can setup a profile that will convert it for me, as well as put the data in a format that is easy to send and secure.

## **Use Case 3**

  

* Let's assume that I own an insurance company, and at this company we need to make use of a lot of integration with other companies, as well as lots of different reports.
